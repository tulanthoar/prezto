...
au VimEnter * let g:volatile_ftypes += ["taglist"]
...
let g:fastfold_skip_filetypes = [ "taglist", "gitcommit", "startify", "man" ]
au FileType vim setl foldmethod=syntax
...
let g:deoplete#auto_refresh_delay = 55
let g:deoplete#file#enable_buffer_path = 1
let g:deoplete#max_menu_width = 100
let g:deoplete#tag#cache_limit_size = 5000000
let g:deoplete#sources = {}
let g:deoplete#sources._ = [ "buffer", "member", "tag", "file", "around", "neosnippet", "include", "syntax" ]
...
au FileType haskell,perl,vim,python :call deoplete#custom#set("file", "rank", 159)
...
au FileType haskell,perl,vim,python :call deoplete#custom#set("around", "rank", 89)
au FileType haskell,perl,vim,python :call deoplete#custom#set("tag", "rank", 85)
au FileType haskell,perl,vim,python :call deoplete#custom#set("neosnippet", "rank", 84)
...
let tlist_perl_settings   = "perl;c:constants;f:formats;l:labels;p:packages;s:subroutines;d:subroutines;o:POD;t:Keyword Comments"
...
au FileType python setl fde=SimpylFold(v:lnum) foldmethod=expr foldlevel=3 nonumber
au FileType python setl def=^\s*\\(def\\\\|class\\) cms=#%s nowrap fo-=t complete+=t
...
au VimEnter * :hi pythonFunctionTag ctermfg=9 ctermbg=23
au VimEnter * :hi pythonMethodTag ctermfg=12 ctermbg=23
au VimEnter * :hi pythonClassTag ctermfg=11 ctermbg=23
au VimEnter * :hi pythonImportedObject ctermfg=10 ctermbg=22
au VimEnter * :hi pythonImportedFuncDef ctermfg=9 ctermbg=22
au VimEnter * :hi pythonImportedClassDef ctermfg=11 ctermbg=22
au VimEnter * :hi link perlFunctionTag pythonFunctionTag
au VimEnter * :hi link vimAutoGroupTag pythonClassTag
au VimEnter * :hi link vimCommandTag pythonFunctionTag
au VimEnter * :hi link vimFuncNameTag pythonMethodTag
au VimEnter * :hi link vimScriptFuncNameTag pythonMethodTag
au VimEnter * :hi SignColumn ctermbg=25
au VimEnter * :hi FoldColumn ctermbg=26
au VimEnter * :hi Visual cterm=inverse ctermbg=233
au VimEnter * :hi WildMenu cterm=bold ctermbg=236
au VimEnter * :hi StatusLine ctermbg=234
au VimEnter * :hi Search ctermbg=53 ctermfg=117
au VimEnter * :hi MatchParen ctermbg=20 ctermfg=25
au VimEnter * :hi Conceal ctermfg=27

exe 'au FileType vim,perl,python :sy match IndentLine %^ % contained conceal cchar='.g:indentLine_first_char
...
if has('conceal') | se cole=2 cocu=nc | endif
...
let g:rg_opts = ['-SL', '--hidden', '-g', '!.git', '-g', '!.gitignore',
            \ '-g', '!.gitsubmodules', '-g', '!.cache']
au VimEnter * :cal denite#custom#var('grep', 'default_ops', ['--vimgrep'] + g:rg_opts)
...
function! YRRunAfterMaps()
    au BufEnter * noremap <silent><expr>g/ incsearch#go(<SID>config_easyfuzzymotion())
    au BufEnter * nmap                  ?  <Plug>(incsearch-fuzzy-stay)
    for [k, c] in items( { 'j' : 'j',
                        ...
                        \ 'w' : 'bd-wl',
                        \ 'e' : 'bd-el', } )
        exe 'au BufEnter * map <expr>'.k.' v:count ? "'.k.'" : "\<Plug>(easymotion-'.c.')"'
    endfor
    for [k, c] in items( { '*' : 'z*',
                        \ 'S' : 'z*',
                        \ '#' : 'z#',
                        ...
                        \ 'gS' : 'gz*',
                        \ 'g#' : 'gz#', } )
        exe 'au BufEnter * nmap '.k.' <Plug>(asterisk-'.c.')<Plug>(easymotion-bd-n)'
        exe 'au BufEnter * omap '.k.' <Plug>(asterisk-'.c.')<Plug>(easymotion-bd-n)'
    endfor
    for [k, c] in items( { 's' : 'bd-f2',
                        \ 'K' : 'sol-bd-jk', } )
        exe 'au BufEnter * map '.k.' <Plug>(easymotion-'.c.')'
    endfor
    for [k, c] in items( { '/' : 'line',
                        \ 'F' : 'grep', } )
        exe 'au BufEnter * map '.k.' :<C-U>Denite -cursor-wrap '.c.'<CR>'
    endfor
endfunction

nmap <unique><M-e> <Plug>MoveLineDown
nmap <unique><M-s> <Plug>MoveLineUp
vmap <unique><M-e> <Plug>MoveBlockDown
vmap <unique><M-s> <Plug>MoveBlockUp
nmap <unique><M-d> <Plug>MoveLineHalfPageDownzz
nmap <unique><M-u> <Plug>MoveLineHalfPageUpzz
vmap <unique><M-d> <Plug>MoveBlockHalfPageDownzz
vmap <unique><M-u> <Plug>MoveBlockHalfPageUpzz
...
nnoremap <silent><unique><leader>cd      :cd %:p:h<CR>

for n in ['1', '2', '3', '4', '5', '6', '7', '8', '9']
    exe 'au VimEnter * nmap <unique>'.g:mapleader.n.' <Plug>AirlineSelectTab'.n
endfor
...
nmap              <unique><leader>t        <Plug>TaskList<Down><Down>
nnoremap <silent><unique><leader>' :SignatureListBufferMarks 1<CR>:lopen 5<CR>:Denite line<CR>
nnoremap  <silent><unique><leader>n
        \ :<C-U>echo(system("tmux if -F '#{s/1/0/:window_panes}'
...
for [k,c] in items({ 'q' : 'unite:quickfix',
                    ...
                    \ ':' : 'command',
                    \ 'F' : 'file_rec:~' })
    exe 'nnoremap  <silent><unique>'.g:mapleader.k.' :Denite -auto-preview -cursor-wrap '.c.'<CR>'
endfor
